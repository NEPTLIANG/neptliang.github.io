...
这是困惑的一个常见来源。很多时候人们会写这样的代码，却不明白为什么会出现错误：
...
尝试
TypeScript 理解该函数user.becomeAdmin有一个对应this的外部对象user。this，呵呵，对于很多情况来说已经足够了，但是在很多情况下，您需要更多地控制对象this代表的内容。JavaScript 规范声明你不能有一个名为 的参数this，因此 TypeScript 使用该语法空间让你this在函数体中声明类型。
...
尝试
这种模式在回调风格的 API 中很常见，其中另一个对象通常控制何时调用您的函数。请注意，您需要使用function而不是箭头函数来获得此行为：
...
void
void表示不返回值的函数的返回值。只要函数没有任何return语句，或者没有从这些返回语句返回任何显式值，它就是推断类型：
...
尝试
在 JavaScript 中，不返回任何值的函数将隐式返回值undefined。但是，void在undefinedTypeScript 中并不是一回事。本章末尾有更多详细信息。

void不一样undefined。

object
特殊类型object是指任何不是原始值（string、number、bigint、boolean、symbol、null或undefined）的值。这与空对象类型 { }不同，也与全局类型不同Object。您很可能永远不会使用Object.

object不是Object。一直用object！

请注意，在 JavaScript 中，函数值是对象：它们具有属性，Object.prototype在其原型链中具有，是instanceof Object，您可以调用Object.keys它们，等等。object出于这个原因，函数类型在 TypeScript中被认为是s。

unknown
unknown类型代表任何值。这类似于any类型，但更安全，因为用值做任何事情是不合法的unknown：
...
尝试
这在描述函数类型时很有用，因为您可以描述接受任何值而any函数体中没有值的函数。
...
尝试
该never类型表示从未观察到的值。在返回类型中，这意味着函数抛出异常或终止程序的执行。

never当 TypeScript 确定联合中没有任何内容时也会出现。
...
尝试
Function
全局类型Function描述了 JavaScript 中所有函数值上的属性，如bind、call、和其他属性。apply它还具有Function总是可以调用类型值的特殊属性；这些调用返回any：

function doSomething(f: Function) {
  return f(1, 2, 3);
}
尝试
这是一个无类型的函数调用，通常最好避免，因为any返回类型不安全。

如果您需要接受任意函数但不打算调用它，则该类型() => void通常更安全。
...
尝试
在 TypeScript 中，这些参数上的类型注释是隐式any[]的而不是any，并且给定的任何类型注释都必须是Array<T>or形式T[]，或者是元组类型（我们稍后会了解）。

休息参数
相反，我们可以使用扩展语法从数组中提供可变数量的参数。例如，push数组的方法接受任意数量的参数：
...
尝试
这种情况的最佳解决方案取决于您的代码，但一般来说，const上下文是最直接的解决方案：
...
const angle = Math.atan2(...args);
尝试
downlevelIteration在针对较旧的运行时时，可能需要打开使用 rest 参数。
...
具有返回类型的上下文类型不会强制函数不void返回某些内容。另一种说法是具有返回类型 ( ) 的上下文函数类型，当实现时，可以返回任何其他值，但将被忽略。voidtype vf = () => void

因此，该类型的以下实现() => void是有效的：
...
尝试
并且当这些函数之一的返回值被分配给另一个变量时，它将保留以下类型void：
...
const v3 = f3();
尝试
存在这种行为，因此即使Array.prototype.push返回一个数字，以下代码也是有效的，并且该Array.prototype.forEach方法需要一个返回类型为 的函数void。
...
尝试
还有另一种特殊情况需要注意，当文字函数定义具有void返回类型时，该函数不得返回任何内容。
...
尝试
有关更多信息，void请参阅这些其他文档条目：

v1 手册
v2 手册
常见问题解答 - “为什么返回非 void 的函数可分配给返回 void 的函数？” -->
...
通过从 `Type` 中选取一组属性 `Keys`（字符串literal或字符串literal的union）来构造一个类型。
